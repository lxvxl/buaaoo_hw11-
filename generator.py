...
def getAm():
    ...
    messages.add(id)
    if type == 0:
        return ' '.join(['am', id, str(random.randint(-1000, 1000)), str(type), getRandPersonId(1), getRandPersonId(1)])
    else :
        return ' '.join(['am', id, str(random.randint(-1000, 1000)), str(type), getRandPersonId(1), getRandGroupId(1)])
